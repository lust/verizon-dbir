...
function arcDiagram(graph) {
    ...
    svg.append("text").text("LINKS TO ACTIONS").attr('class', 'label').style('text-anchor', 'middle').attr('transform', 'translate(' + width / 2 + ',' + margin + ')');
    //
    var wp = height - margin;
    svg.append("text").text("LINKS TO ATTRIBUTES").attr('class', 'label').style('text-anchor', 'middle').attr('transform', 'translate(' + width / 2 + ',' + wp + ')');

    svg.append("text").text("INCIDENT COUNT").attr('class', 'label').style('text-anchor', 'end').attr('dy', '.35em').attr('transform', 'translate(' + offset + ',' + height / 2 + ')');
...
function drawNodes(nodes) {
    ...
    var nodeScale = d3.scale.linear().domain([min, max]).range([6, radius]);
    ...
    }).attr('class', 'nodeCount').style('pointer-events', 'none').style('text-anchor', 'middle').style('opacity', 0).attr('dy', '.35em').attr('transform', function (d) {
